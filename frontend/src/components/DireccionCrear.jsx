...
export default function DireccionCrear() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await direccionService.crear(payload, user.accessToken)

            if (result.ok) {
                sessionStorage.setItem('alerta_tipo', 'success')
                sessionStorage.setItem('alerta_msg', 'Dirección creada exitosamente.')
                navigate(`/terceros/${terceroId}/direcciones`)
            } else {
                setMensajeError(result.message)
            }
        } catch {
            setMensajeError('Error de conexión al guardar la dirección. Intente nuevamente.')
        } finally {
            setGuardando(false)
        }
    }
        ...
        <>
            ...
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800 page-header-title">
                    <i className="fas fa-map-marker-alt text-primary" aria-hidden="true" /> Nueva Dirección
                </h1>
                <button
                            ...
                            <div className="col-md-12">
                                <div className="form-group">
                                    ...
                                    <textarea
                                        ...
                                        onChange={cambiar}
                                        disabled={guardando}
                                        placeholder="100m sur del parque, casa amarilla con portón verde..."
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="form-group text-right mb-0 form-actions">
                            ...
                            >
                                {guardando ? (
                                    ...
                                    <>
                                        <i className="fas fa-save" aria-hidden="true" /> Guardar Dirección
                                    </>
                                )}
                            </button>
                        </div>
