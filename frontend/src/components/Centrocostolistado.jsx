...
// Helpers de estado de asiento → badge color
...
function FilaLinea({ linea, onDistribuir }) {
    const saldo = (linea.monto ?? 0) - (linea.montoProrrateado ?? 0)
    const saldoCero = Math.abs(saldo) < 0.009
    const estadoCodigo = (linea.estadoCodigo ?? '').toUpperCase().trim()
    const puedeProrratear = estadoCodigo === 'EA3' || estadoCodigo === 'EA4'
    ...
    return (
        <tr>
            ...
            <td className="align-middle text-right">
                ₡ {Number(linea.montoProrrateado ?? 0).toLocaleString('es-CR', { minimumFractionDigits: 2 })}
            </td>
            <td className="align-middle text-right">
            ...
            <td className="align-middle text-center">{linea.cantidadCentros}</td>
            <td className="align-middle text-center">
                {puedeProrratear ? (
                    <button
                        className="btn btn-sm btn-primary"
                        onClick={() => onDistribuir(linea.detalleId)}
                        title="Distribuir por centro de costo"
                    >
                        <i className="fas fa-sitemap" aria-hidden="true" /> Distribuir
                    </button>
                ) : (
...
export default function CentroCostoListado() {
    const { user } = useAuth()
    ...
    useEffect(() => {
        async function cargarPeriodos() {
            try {
                const data = await centroCostoService.listarPeriodos(user.accessToken)
    ...
    // Cargar líneas cuando cambia el período
    const cargarLineas = useCallback(async (id) => {
        if (!id) return
        setCargando(true)
        setErrorCarga('')
        try {
            const data = await centroCostoService.listarLineas(id, user.accessToken)
            if (data.statusCode === 200) {
        ...
        <>
            ...
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800">
                    <i className="fas fa-project-diagram text-primary" aria-hidden="true" />{' '}
                    ...
                    {!cargando && errorCarga && (
                        <div className="alert alert-danger" role="alert">
                            ...
                            <button
                                ...
                                onClick={() => cargarLineas(periodoId)}
                            >
                                Reintentar
                            </button>
                        </div>
                            ...
                            >
                                <thead className="thead-light">
                                    <tr>
                                        ...
                                        <th className="text-center">Centros</th>
                                    ...
                                    {lineas.length === 0 ? (
                                        <tr>
                                            <td colSpan={10} className="text-center text-muted">
                                                ...
                                                <p className="mb-0">
                                                    No hay líneas de asiento para el período seleccionado.
                                                </p>
                                            </td>
                                        </tr>
                                    ) : (
                                        lineas.map((linea) => (
                                            <FilaLinea
                                                ...
                                                onDistribuir={(detalleId) =>
                                                    navigate(`/centro-costo/prorrateo/${detalleId}`)
                                                }
                                            />
                                        ))
                                    )}
