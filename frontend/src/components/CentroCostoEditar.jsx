...
export default function CentroCostoAdminEditar() {
    const { user } = useAuth()
    ...
    // Cargar datos del registro al montar
    useEffect(() => {
        async function cargar() {
            setCargando(true)
            setErrorCarga('')
            try {
                const data = await centroCostoService.obtenerPorCodigo(codigo, user.accessToken)
                if (data.statusCode === 200) {
                    const cc = data.responseObject
                    setForm({
                        codigo: cc.codigo ?? '',
                        nombre: cc.nombre ?? '',
                    ...
                    setErrorCarga('Centro de costo no encontrado.')
                } else {
                    setErrorCarga(data.message ?? 'Error al cargar el centro de costo.')
                }
            } catch {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // El endpoint PUT usa el código original de la URL
            const data = await centroCostoService.actualizar(codigo, payload, user.accessToken)

            if (data.statusCode === 200) {
                guardarAlerta('success', 'Centro de costo actualizado exitosamente.')
                navigate('/centro-costo/admin')
            } else {
                setErrorServidor(data.message ?? 'Error al actualizar el centro de costo.')
            }
        } catch {
        ...
        <>
            ...
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800">
                    <i className="fas fa-edit text-primary" aria-hidden="true" /> Editar Centro de Costo
                </h1>
                <button
