...
// Valores hardcodeados de tipos/estados — el microservicio los tiene como catálogos
// pero el PHP también los tenía hardcodeados en la vista. Se mantiene idéntico.
...
function validar(datos) {
    ...
    } else if (datos.identificacion.trim().length > 50) {
        errores.push('La identificación no puede superar 50 caracteres.')
    }
...
export default function TerceroCrear() {
    const { user } = useAuth()
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await terceroService.crear(payload, user.accessToken)

            if (data.statusCode === 201) {
                sessionStorage.setItem('alerta_tipo', 'success')
                sessionStorage.setItem('alerta_msg', 'Tercero creado exitosamente.')
                navigate('/terceros')
            } else if (data.statusCode === 409) {
                setMensajeError('Ya existe un tercero con esa identificación.')
            } else if (data.statusCode === 400) {
                setMensajeError(data.message ?? 'Datos inválidos.')
            } else {
                setMensajeError(data.message ?? 'Error al guardar el tercero. Intente nuevamente.')
            }
        } catch {
            setMensajeError('Error de conexión al guardar el tercero. Intente nuevamente.')
        } finally {
            setGuardando(false)
        }
    }
        ...
        <>
            ...
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800 page-header-title">
                    <i className="fas fa-user-plus text-primary" aria-hidden="true" /> Nuevo Tercero
                </h1>
                <button
                        ...
                        <div className="row">
                            <div className="col-md-6">
                                <div className="form-group">
                                    <label htmlFor="identificacion" className="font-weight-bold">
                        ...
                        <div className="form-group text-right mb-0 form-actions">
                            ...
                            >
                                {guardando ? (
                                    ...
                                    <>
                                        <i className="fas fa-save" aria-hidden="true" /> Guardar Tercero
                                    </>
                                )}
                            </button>
                        </div>
