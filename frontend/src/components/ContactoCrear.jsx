...
export default function ContactoCrear() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const result = await contactoService.crear(payload, user.accessToken)

            if (result.ok) {
                sessionStorage.setItem('alerta_tipo', 'success')
                sessionStorage.setItem('alerta_msg', 'Contacto creado exitosamente.')
                navigate(`/terceros/${terceroId}/contactos`)
            } else {
                setMensajeError(result.message)
            }
        } catch {
            setMensajeError('Error de conexión al guardar el contacto. Intente nuevamente.')
        } finally {
            setGuardando(false)
        }
    }
        ...
        <>
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800 page-header-title">
                    <i className="fas fa-address-book text-primary" aria-hidden="true" /> Nuevo Contacto
                </h1>
                <button
                            ...
                            <div className="col-md-6">
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        disabled={guardando} autoFocus placeholder="Nombre del contacto"
                                    />
                                </div>
                            </div>
                            <div className="col-md-6">
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        maxLength={100} value={datos.cargo} onChange={cambiar}
                                        disabled={guardando} placeholder="Gerente, Encargado..."
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="form-group text-right mb-0 form-actions">
                            ...
                            <button type="submit" className="btn btn-primary" disabled={guardando}>
                                {guardando ? (
                                    ...
                                    <><i className="fas fa-save" aria-hidden="true" /> Guardar Contacto</>
                                )}
                            </button>
                        </div>
                    </form>
